...
dependencies = ["opencv-python", "Pillow", "pygame"]
for package in dependencies:
    check_install(package)

# After ensuring dependencies are installed, import them
import pygame  # We'll use pygame for sound playing
    ...
    def detect_motion(self):
        ...
        while self.is_running:
            ...
            if motion_score > (100 - self.sensitivity) * 255 / 100:
                ...
                # Check if 10 seconds have passed since motion was last detected
                if self.motion_detected_time and time.time() - self.motion_detected_time < 10:
                    self.set_motion_indicator(True)  # Keep red for 10 seconds
                else:
                    self.set_motion_indicator(False)  # Green = No Motion
    ...
    def play_sound(self, sound_file):
        ...
        if not self.sound_playing:
            try:
                ...
                def reset_sound_playing():
                    self.sound_playing = False

                pygame.mixer.music.set_endevent(pygame.USEREVENT)
                pygame.event.set_allowed(pygame.USEREVENT)
                threading.Thread(target=self.check_sound_end, args=(reset_sound_playing,)).start()

            except Exception as e:
                print(f"Error playing sound: {e}")

    def check_sound_end(self, callback):
        """Waits for the sound to finish and calls the callback."""
